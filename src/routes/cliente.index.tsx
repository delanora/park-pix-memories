...
export const Route = createFileRoute("/cliente/")({
  head: () => ({ meta: [{ title: "Minhas fotos | ParkSnap" }] }),
  component: () => (
...
function CustomerPage() {
  ...
  async function downloadOne(photoId: string) {
    try {
      ...
      triggerDownload(blob, `parksnap-${photoId}.jpg`);
    } catch (err: any) {
  ...
  async function downloadAll() {
    ...
    try {
      ...
      for (const p of data) {
        ...
        zip.file(`parksnap-${p.id}.jpg`, buf);
      }
      const blob = await zip.generateAsync({ type: "blob" });
      triggerDownload(blob, "parksnap-todas.zip");
    ...
    <div className="p-6 md:p-8">
      <div className="mb-6 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Todas as fotos que você adquiriu no parque.
          </p>
        </div>
        <Button
          onClick={downloadAll}
      ...
      {isLoading ? (
        ...
        <div className="rounded-2xl border border-dashed border-border bg-card/50 p-10 text-center text-sm text-muted-foreground">
          Você ainda não tem fotos liberadas. Procure um operador no parque.
        </div>
      ) : (
